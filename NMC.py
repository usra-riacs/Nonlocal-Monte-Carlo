...
class NMC:
    ...
    def LBP_convexified(self, lambda_start, lambda_end, lambda_reduction_factor, m_star, epsilon, tolerance,
        ...
        - m_star (np.array, shape (N,)): Initial guess for the magnetizations (marginal probabilities).
        - epsilon (np.array, shape (N,)): Epsilon values for soft-clamping.
        - tolerance (float): Tolerance level for convergence of LBP.
        - max_iterations (int): Maximum number of iterations for LBP.
        - threshold_initial (float): Initial threshold for cluster formation.
        - threshold_cutoff (float): Final threshold for cluster formation.
    ...
    def find_clusters(self, magnetizations, threshold_initial, threshold_cutoff, threshold_step):
        ...
        - threshold_initial (float): Initial threshold for identifying seed spins.
        - threshold_cutoff (float): Cutoff threshold below which clusters won't be expanded.
    ...
    def NMC_subroutine(self, m_star, num_cycles, num_sweeps_per_NMC_phase, full_update_frequency, M_skip, global_beta,
        ...
        - global_beta (float): Factor modifying the temperature.
        - temp_x (float): Temperature modifier for spin groups.
        - lambda_start (float): Initial value for lambda in LBP_convexified.
        - lambda_end (float): Final value for lambda in LBP_convexified.
        - lambda_reduction_factor (float): Factor by which lambda is reduced in each iteration of LBP_convexified.
        - threshold_initial (float): Initial threshold for the LBP_convexified method.
        - threshold_cutoff (float): Cutoff threshold value for the LBP_convexified method.
        - max_iterations (int): Maximum number of iterations allowed in LBP_convexified.
        - tolerance (float): Tolerance value for convergence in LBP_convexified.
        ...
        for cycle in range(num_cycles):
            ...
            h_c[non_clusters] = m_init[
                                    non_clusters] * 10000  # Strongly bias the non-cluster spins to keep their current values

            M = self.MCMC_GC(num_sweeps_per_NMC_phase, m_init.copy(), global_beta, J_c, h_c, self.colorMap,
                             anneal=False, hash_table= hash_table,use_hash_table=use_hash_table)  # Run MCMC for clusters
            ...
            h_nc[all_clusters] = m_init[
                                     all_clusters] * 10000  # Strongly bias the cluster spins to keep their current values

            M = self.MCMC_GC(num_sweeps_per_NMC_phase, m_init.copy(), global_beta, J_nc, h_nc, self.colorMap,
                             anneal=False, hash_table= hash_table,use_hash_table=use_hash_table)  # Run MCMC for non-clusters
            energies = [- (M[:, i].T @ self.J @ M[:, i] / 2 + M[:, i].T @ self.h) for i in
                        range(M.shape[1])]  # Compute energies
            ...
            if cycle % full_update_frequency == 0:
                ...
                # If clusters are not provided, re-compute clusters after full update
                if not clusters_provided:
                    clusters, _, _, _, _ = self.LBP_convexified(
                        lambda_start, lambda_end, lambda_reduction_factor,  m_star.copy(),
                        epsilon, tolerance, max_iterations, threshold_initial, threshold_cutoff, global_beta
    ...
    def run(self, num_sweeps_initial=int(1e4), num_sweeps_per_NMC_phase=int(1e4),
            num_NMC_cycles=10, full_update_frequency=1, M_skip=1, temp_x=10,
            global_beta=3, lambda_start=3, lambda_end=0.01, lambda_reduction_factor=0.9,
            threshold_initial=0.999999, threshold_cutoff=0.9999, max_iterations=1000, tolerance=np.finfo(float).eps,
            use_hash_table=0):
        """
        Execute the NMC run method
        ...
        - global_beta (float): Global beta for MCMC and NMC
        ...
        M = self.MCMC_GC(num_sweeps_initial, m_init.copy(), global_beta, self.J, self.h, self.colorMap, anneal=True, sweeps_per_beta=1, initial_beta=0,
        ...
        M_overall, energy_overall, min_energy, all_clusters = self.NMC_subroutine(m_star, num_NMC_cycles,
                                                                                  ...
                                                                                  max_iterations,
                                                                                  tolerance, hash_table= hash_table,use_hash_table=use_hash_table)
...
def main():
    ...
    use_hash_table = 1 # Flag to decide whether to use hash table
